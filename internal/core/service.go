...
func (i *Input) Validate(prev *Chore) error {
	...
	if prev != nil && prev.ChoreType != i.ChoreType {
		return fmt.Errorf("can't change choreType from %s to %s", prev.ChoreType, i.ChoreType)
	}
	switch i.ChoreType {
	case "interval":
		if !i.Date.IsZero() {
	...
	case "date":
		...
		if !i.Interval.Zero() {
			return fmt.Errorf("date chore can't have an interval")
		}
		if i.Repeats != 1 {
			return fmt.Errorf("date chore can't have repeats")
		}
	default:
